...
export default function NumberMemoryGame() {
  const [gameState, setGameState] = useState('start'); // start, showing, input, correct, wrong, results
  ...
  const handleSubmit = () => {
    ...
    if (userInput === currentNumber) {
      ...
      if (newLives === 0) {
        ...
        setGameState('wrong');
        // Try same level again after 1.5 seconds
        setTimeout(() => {
          startLevel(level);
        }, 1500);
      }
  ...
  return (
    ...
    >
      <div className="h-full bg-gradient-to-br from-green-500 to-emerald-600 flex flex-col items-center justify-center rounded-3xl min-h-[600px] relative">
        <div className="text-center text-white px-4">
          ...
          {gameState === 'start' && (
            <>
              ...
              <h1 className="text-6xl font-bold mb-6">Number Memory</h1>
              <p className="text-2xl mb-4 opacity-90">
                Memorize the number that appears on screen.
              </p>
              <p className="text-xl mb-2 opacity-75">
              ...
              <button 
                onClick={startGame}
                className="px-8 py-4 bg-white text-green-600 rounded-xl font-bold text-xl hover:bg-opacity-90 transition transform hover:scale-105"
              >
                Start Game
              </button>
            </>
          ...
          {gameState === 'showing' && (
            <>
              <div className="mb-8">
                <p className="text-2xl opacity-75 mb-4">Level {level}</p>
              </div>
          ...
          {gameState === 'input' && (
            <>
              ...
              <h2 className="text-4xl font-bold mb-8">What was the number?</h2>
              <input
                ref={inputRef}
                type="text"
                inputMode="numeric"
                ...
                className="w-80 max-w-full px-6 py-4 text-3xl text-center rounded-xl bg-white text-gray-900 font-bold mb-6 focus:outline-none focus:ring-4 focus:ring-white/50"
              ...
              <button 
                ...
                disabled={!userInput}
                className="px-8 py-4 bg-white text-green-600 rounded-xl font-bold text-xl hover:bg-opacity-90 transition transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
              >
                Submit
              </button>
            </>
          ...
          {gameState === 'correct' && (
            <>
              ...
              <h1 className="text-6xl font-bold mb-4">Correct!</h1>
              <p className="text-2xl opacity-90">Level {level - 1} complete</p>
              <p className="text-xl opacity-75 mt-2">Next: {level} digits</p>
            </>
          )}
          ...
          {gameState === 'wrong' && (
            <>
              ...
              <h1 className="text-6xl font-bold mb-4">Incorrect!</h1>
              <p className="text-3xl mb-2">The number was: <span className="font-mono">{currentNumber}</span></p>
              <p className="text-2xl opacity-90 mb-2">Lives remaining: {lives}</p>
              <p className="text-xl opacity-75">Try again...</p>
            </>
          )}
          ...
          {gameState === 'results' && (
            <>
              ...
              <h1 className="text-7xl font-bold mb-6">{highestLevel}</h1>
              <p className="text-3xl mb-8 opacity-90">Highest Level Reached</p>
              
              <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 mb-8 max-w-md mx-auto">
                <p className="text-xl mb-2">You remembered up to:</p>
                <p className="text-4xl font-bold mb-4">{highestLevel} digits</p>
        ...
        {gameState !== 'start' && gameState !== 'results' && (
          <div className="absolute top-8 left-8">
            <div className="bg-white/20 backdrop-blur-sm rounded-lg px-4 py-2">
              ...
              <p className="text-white/80 text-sm">{level} {level === 1 ? 'digit' : 'digits'}</p>
            </div>
          </div>
        )}
      </div>
