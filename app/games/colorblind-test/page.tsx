...
// Fisher-Yates shuffle (to ensure no repeats)
...
export default function ColorBlindTest() {
  ...
  const [order, setOrder] = useState([]); // NEW — randomized plate order

  // Hardcoded mapping: image filename → correct answer
  const plates = [
  ...
  const startTest = () => {
    const plateOrder = shuffleArray([...Array(plates.length).keys()]); // random order 0–9
  ...
  const handleNumberClick = (number: string) => {
    ...
    setAnswers(prev => [
      ...prev,
      {
        plate: realIndex, // REAL image index
        userAnswer: number,
  ...
  return (
    ...
    >
      <div className="h-full bg-gradient-to-br from-indigo-900 to-purple-900 flex flex-col items-center justify-center rounded-3xl min-h-[600px] p-8">
        ...
        {gameState === 'start' && (
          <div className="text-center text-white max-w-2xl">
            ...
            <h1 className="text-6xl font-bold mb-6">Color Blind Test</h1>
            <p className="text-xl mb-4 opacity-90">
              Screen for color vision deficiencies with Ishihara test plates.
            </p>
            <div className="bg-yellow-500/20 border border-yellow-500/40 rounded-xl p-4 mb-8">
        ...
        {gameState === 'testing' && (
          <div className="text-center text-white max-w-2xl">
            ...
            <div className="mb-6">
              <p className="text-lg opacity-75 mb-2">Plate {currentPlate + 1} of {totalPlates}</p>
            ...
            {/* Ishihara Plate */}
            <div className="mb-6">
              <div className="relative w-80 h-80 mx-auto bg-white rounded-full overflow-hidden shadow-2xl">
                <Image
            ...
            <h2 className="text-2xl font-bold mb-6">What number do you see?</h2>
            ...
            <div className="grid grid-cols-5 gap-3 max-w-md mx-auto mb-6">
              {[1, 2, 3, 4, 5, 6, 7, 8, 9, 0].map((num) => (
                <button
                  ...
                  className="aspect-square bg-white/10 hover:bg-white/20 border-2 border-white/20 hover:border-white/40 rounded-xl text-3xl font-bold text-white transition transform hover:scale-105 active:scale-95"
                >
                  {num}
                </button>
              ))}
        ...
        {gameState === 'results' && result && (
          <div className="text-center text-white max-w-2xl">
            ...
            <h1 className={`text-5xl font-bold mb-4 ${result.color}`}>
              {result.status}
            </h1>
            <p className="text-xl mb-8 opacity-90">{result.message}</p>

            <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 mb-8">
              <div className="text-center mb-6">
                <p className="text-6xl font-bold mb-2">{correctAnswers}/{totalPlates}</p>
                <p className="text-xl opacity-75">Correct Answers</p>
              ...
              <div className="space-y-2 text-left max-h-60 overflow-y-auto">
                {answers.map((answer, index) => (
                  <div key={index} className="flex items-center justify-between text-sm border-b border-white/10 pb-2">
